...
class OrderRepository extends RepositoryBase {
  ...
  */
  ...
  async analytics (restaurantId) {
    ...
    const [numYesterdayOrders, numPendingOrders, numDeliveredTodayOrders, invoicedToday] = await Promise.all([
      OrderSequelize.count({
        where: {
          createdAt: { [Op.gte]: yesterdayZeroHours, [Op.lt]: todayZeroHours },
          restaurantId
        }
      }),
      OrderSequelize.count({

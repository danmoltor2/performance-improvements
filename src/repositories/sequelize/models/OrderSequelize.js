...
const loadModel = (sequelize, DataTypes) => {
  class OrderSequelize extends Model {
    ...
    */
    ...
    toJSON () {
      ...
      if (this.products) {
        attributes.products = this.products.map(product => ({
          ...product.toJSON(),
  ...
  OrderSequelize.init({
    ...
    indexes: [
      ...
      { fields: ['restaurantId', 'startedAt'] },
      { fields: ['restaurantId', 'deliveredAt'] }
    ]
  })
...
const flattenOrderProducts = orders => {
  ...
  Array.isArray(orders) ? orders.forEach(normalize) : normalize(orders)
}
